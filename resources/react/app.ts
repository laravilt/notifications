 ...
 */
function processNotificationCookie(): void {
 ...
 */
export function setupNotificationCookieCheck(): void {
    if (typeof window === 'undefined') {
        return;
    }

    if (cookieCheckInitialized) {
        return;
    }
    ...
    window.fetch = async function (this: any, ...args: Parameters<typeof fetch>): Promise<Response> {
...
/**
 * The Vue package registers no global components; `register()` only makes sure the cookie check
 * is running (it is idempotent and also starts automatically on import).
 */
export default {
    register(): void {
        setupNotificationCookieCheck();
